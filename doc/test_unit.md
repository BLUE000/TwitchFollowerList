...
- **入力**: `"1,Nickname,TestUser,test_user,12345,"`
- **期待結果**:
...
### 2.3 `nickname` / `memo` フィールドの保存と復元
- **テスト内容**: ニックネームやメモに含まれるカンマや特殊文字が正しく保存・復元されるか。
- **入力**: `nickname = "ニック,ネーム"`, `memo = "テスト, メモ\n改行あり"` を持つフォロワー。
- **期待結果**: 保存されたファイルを `decodeData` して再度パースした際、各フィールドの内容が完全に一致すること。
