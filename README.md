...
1. **ZIPをダウンロード**: 最新の `FollowerList_v2.0.zip` をダウンロードして展開します。
2. **ファイルを上書き**: 展開されたフォルダ内にある以下のファイルを、現在お使いのプログラムフォルダへ上書きコピーします。
    - `FollowerList.exe`
    - `libTransCipher.dll`
3. **（推奨）データのバックアップ**: アップデート前に `out` フォルダと `Config` フォルダをコピーしてバックアップしておくことをお勧めします。
    - ※ `out` フォルダにはあなたのフォロワーリスト（`.dat`ファイル）が、`Config` フォルダには設定やログイン情報が含まれています。これらを削除・上書きしなければ、以前のデータはそのまま保持されます。
