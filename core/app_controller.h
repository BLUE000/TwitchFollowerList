...
/**
 * @class AppController
 * @brief UI（MainWindow）と各 API
 * サービスを仲介し、ビジネスロジックを制御するクラス。
 */
class AppController : public QObject {
  Q_OBJECT
public:
  static const int LOGIN_TIMEOUT_MS = 30000; ///< ログインタイムアウト（30秒）
  static const int SMART_REFRESH_WAIT_MS =
      500; ///< 更新演出用のウェイト（0.5秒）
  /**
   * @brief コンストラクタ。
...
public slots:
   ...
   * @param iGrpId グループ ID。
   */
  void handleFollowersUnassignedFromGroup(const QStringList &lstUsrIds,
                                          int iGrpId);
